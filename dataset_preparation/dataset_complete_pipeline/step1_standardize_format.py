...
Sangraha, Wikipedia and FineWeb-Edu each ship with a different schema.
This script reads every downloaded shard and rewrites it into one canonical
document schema so the rest of the pipeline (quality filtering, dedup,
tokenize+pack) only ever has to deal with a single shape:
...
Input:  <repo>/../dataset/{sangraha,wikipedia,fineweb-edu}/...
Output: <repo>/../dataset/standardized/<language>/<source>/part-XXXXX.parquet

Paths are resolved relative to this file so nothing personal/absolute ever
gets hardcoded here.
"""
...
def main() -> None:
    ...
    print(f"\nDone. Standardized output written under: dataset/standardized/")
